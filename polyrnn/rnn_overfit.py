...
train_data, valid_data = get_train_and_valid_datasets('/home/wesley/data/tiny-polygons', max_timesteps=max_timesteps,
                                                      ...
                                                      history_length=0, is_local=True, load_max_images=1)
...
with tf.Session() as sess:
    ...
    for _ in range(total_steps):
        step, loss, accuracy, p, _ = sess.run([global_step, loss_op, accuracy_op, predictions_sparse, train_op],
                                              feed_dict={image_pl: batch_images, duration_pl: batch_d,
                                                         targets_pl: batch_t})
        print('Step {}/{}\tLoss={}\tAccuracy={}\t{}'.format(step, total_steps, loss, accuracy, p))
