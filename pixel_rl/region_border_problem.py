...
predictions = tf.placeholder(tf.float32, dtype=tf.float32, shape=[None, 32, 32])
ground_truth = tf.placeholder(tf.float32, dtype=tf.float32, shape=[None, 32, 32])
tp = tf.reduce_sum(predictions * ground_truth)
fp = tf.reduce_sum(predictions * (1 - ground_truth))
fn = tf.reduce_sum((1 - predictions) * ground_truth)
iou = tp / (tp + fp + fn)
