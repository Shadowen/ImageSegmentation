...
parser.add_argument('-w', '--num-workers', default=4, type=int, help="Number of workers")
...
def create_commands(session, num_workers, logdir, shell='bash', mode='tmux', visualise=False):
    # for launching the TF workers and for launching tensorboard
    base_cmd = ['CUDA_VISIBLE_DEVICES=', sys.executable, 'worker.py', '--log-dir', logdir,
    ...
    for i in range(num_workers):
        cmds_map += [new_cmd(session, "w-{}".format(i),
                             base_cmd + ["--job-name", "worker", "--task", str(i)], mode, logdir, shell)]
    ...
    cmds = ["mkdir -p {}".format(logdir), "echo {} {} > {}/cmd.sh".format(sys.executable,
                                                                          ' '.join(
                                                                              [shlex_quote(arg) for arg in sys.argv if
                                                                               arg != '-n']), logdir), ]
...
def run():
    ...
    logdir = '/data/PolyRLOverfitExperiment'

    cmds, notes = create_commands("a3c", args.num_workers, logdir, mode=args.mode,
