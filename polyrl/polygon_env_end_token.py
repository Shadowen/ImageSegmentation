...
class PolygonEnv():
    ...
    def step(self, action):
        ...
        if np.all(action == [self.dataset._prediction_size, 0]):
            self.predicted_vertices.append(self.vertices[0])
...
if __name__ == '__main__':
    ...
    show_state(state)
    for timestep in range(training_env.dataset._history_length, len(training_env.vertices)):
        print("t={}".format(timestep))
        a = training_env.vertices[timestep % len(training_env.vertices)]
        state, reward, done = training_env.step(action=([a[0], a[1]], 0))
        print(reward)
        show_state(state)
    print("t={}".format(timestep))
    a = training_env.vertices[timestep % len(training_env.vertices)]
    state, reward, done = training_env.step(action=([0, 0], 1))
    print(reward)
    show_state(state)
    plt.show(block=True)
